...
export const PAYMENT_REJECTED_TEMPLATE_NAME = process.env.WHATSAPP_PAYMENT_REJECTED_TEMPLATE || 'seubeat_pagamento_rejeitado_v1';
export const VIDEO_UPSELL_TEMPLATE_NAME = process.env.WHATSAPP_VIDEO_UPSELL_TEMPLATE || 'seubeat_video_upsell_v6';
...
export function listTemplates(): Array<{ bucket: string; name: string; language: string }> {
  ...
  abandoned.push({ bucket: 'payment_rejected', name: PAYMENT_REJECTED_TEMPLATE_NAME, language: TEMPLATE_LANGUAGE });
  abandoned.push({ bucket: 'video_upsell', name: VIDEO_UPSELL_TEMPLATE_NAME, language: TEMPLATE_LANGUAGE });
  return abandoned;
}

// Buckets com envio por WhatsApp (env WHATSAPP_ENABLED_BUCKETS, default '30min,24h,48h,72h').
// Todos os buckets de abandono enviam WhatsApp por defeito.
export function enabledWhatsAppBuckets(): AbandonedBucketKey[] {
  const raw = process.env.WHATSAPP_ENABLED_BUCKETS;
  const enabled = (raw && raw.trim() !== '' ? raw : '30min,24h,48h,72h').split(',').map((s) => s.trim()) as AbandonedBucketKey[];

...
## Progress
### Done
- **Bugfix: reenvio de comprovativo após rejeição (18/Ago 2026)**: cliente com pagamento rejeitado (`payment_rejected` + linha `payments.status='rejected'`) não conseguia reenviar — `/submit-payment` fazia sempre `INSERT` em `payments`, mas `request_id` é UNIQUE e a linha rejeitada já existia → 500 com rollback para `payment_rejected`. Confirmado em produção (13/Ago, `jose.tome2027@gmail.com`, 7 tentativas falhadas + 8 ficheiros órfãos no storage). Fix em `server/routes/public.ts`: antes do INSERT, consulta-se payment `rejected` para o `request_id`; se existir, faz `UPDATE payments SET status='pending_verification', notes=null, approved_at=null, proof_*` na linha existente (em vez de INSERT), mantendo UNIQUE. Rollback partilhado nos dois caminhos. +1 teste em `submit-payment.test.ts` (7 total; mock extendido com filtro `status=rejected`). **Suite: 323 testes** (31 ficheiros) — todos passam; `tsc --noEmit`/lint passam. Limpeza manual em produção via Storage API: 8 órfãos `proofs/178662*` apagados, comprovativo real (`1786625828444_IMG_8281.jpeg`, referenciado pelo payment aprovado) mantido.
...
- **Clonagem de voz Premium funcional (17/Ago 2026)**: em produção a voz nunca era realmente clonada — 3/3 falhas (`{"failed":true}`, `"It didn't sound like you said the phrase"`): a API sunoapi.org rejeita amostras que não contenham a frase de validação gerada, e o cliente nunca lia a frase. Fix completo:
  ...
  - Testes: `server/__tests__/voice-validation-phrase.test.ts` (5), `process-suno-voice.test.ts` (5: reutilização com pré-verificação, fallback legado, falha→degradação, task expirada→frase nova, frase não corresponde→descarta), `submit-payment.test.ts` (7, com frase guardada). Suite completa **323 testes** (31 ficheiros) — todos passam em run paralelo; `tsc --noEmit`/lint passam.
...
## Testes
- **323 testes**, 31 ficheiros — todos passam em run paralelo (~45s).- Distribuição: validation (21), email-utils (15), suno-utils (20), AdminPanel (26), validation-frontend (20), SongPlayer (8), metaPixel (20), song-api (11), useAudioPlayer (4), smoke (1), metaPixelCapi (2), ai (5), aiShared (14), helpers (5), workflow-rollback (8), abandoned-messages (22), abandoned-whatsapp-route (8), whatsapp-cloud (21), uuid (5), aiFailure (6), failed-lyrics-recovery (10), admin-regenerate (3), submit-payment (7), abandoned-scheduler (10), deepseek (5), admin-fixes (9), voice-validation-phrase (5), process-suno-voice (5), metaAds (2), utm (4), deepseek-config (2).
...
## Funil de conversão (13/Ago 2026) — análise read-only
- **Volume**: 217 pedidos ativos (não apagados). Criação diária recente: 6–36/dia (pico 06/Ago: 36; gap 21–24 e 27–31 Jul = 0 criados → possível pausa/outage).
- **Funnel**: 217 criados → 184 com letras (`lyrics_ready`, 85%) → **16 submetem pagamento (8.7% dos com letras)** → 13 aprovados (81% dos que pagaram) → **13 entregues (100% dos aprovados)**. 3 rejeitados (todos nos últimos 14 dias).
...
- `server/__tests__/submit-payment.test.ts`: 7 testes do guard anti-rebaixamento + rollback + reenvio pós-rejeição (UPDATE em vez de INSERT) + `validation_task_id`/frase do `POST /submit-payment`.
